...
export interface RobotInstanceConnectionStatusPanelProps {
  /** `page`: standalone Connection status screen. `embedded`: inside Endpoints workspace. */
  layout?: 'page' | 'embedded';
  titleKey?: string;
  leadKey?: string;
}

export function RobotInstanceConnectionStatusPanel({
  layout = 'embedded',
  titleKey = 'support.robot.connections.status.title',
  leadKey = 'support.robot.connections.status.lead',
}: RobotInstanceConnectionStatusPanelProps) {
  ...
  const columns: ColumnsType<RobotConnectionStatusRow> = [
    {
      title: t('support.robot.connections.status.col.instanceAndTarget'),
      key: 'robotEndpoint',
      width: 280,
      render: (_, row) => {
  ...
  return (
    <div className={embedded ? 'robot-instance-status-panel robot-instance-status-panel--embedded' : undefined}>
      {!embedded ? (
        ...
        <div className="dev-data-foundry-header" style={{ marginBottom: 12 }}>
          <div>
            <Typography.Title level={4} className="domain-content-title" style={{ margin: 0 }}>
              {t('support.robot.connections.endpoints.statusSection')}
            </Typography.Title>
            <Typography.Paragraph type="secondary" className="domain-1depth-page-lead" style={{ marginBottom: 0 }}>
              {t('support.robot.connections.endpoints.statusSectionLead')}
            </Typography.Paragraph>
          </div>
          <Space wrap>
            <Space align="center">
      ...
      <Card size="small" styles={{ body: { padding: 0 } }} style={{ borderColor: token.colorBorderSecondary }}>
        <Table<RobotConnectionStatusRow>
          ...
          onRow={(record) => ({
            onClick: () =>
              navigate(supportRobotWorkspaceDetailPath('instances-endpoints', record.endpointId), {
